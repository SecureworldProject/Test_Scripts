/*
   Creates and populates a test file for the other project to work.
*/
...
int main(void) {
    ...
    // Note that this code does not check for storage medium overflow or other errors, which production code should do.
        // If the 1st loop is used, provides unique int (4-byte) offsets in the file for easy visual inspection.
        // If the 2nd loop is used, provides a file full of the letter provided in the "letter" variable.
    _tprintf(TEXT("CREATING FILE...\n"));

    // Fill with unique ints
    /*for (i = 0; i < (int)dwSysGran; i++) {
        WriteFile(hFile, &i, sizeof(i), &dBytesWritten, NULL);
    }*/

    // Fill with provided letter
    letter = 'C';
    for (i = 0; i < BUFFSIZE; i++) {
        WriteFile(hFile, &letter, sizeof(letter), &dBytesWritten, NULL);
    }
